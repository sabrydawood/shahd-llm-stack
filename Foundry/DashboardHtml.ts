...
<script>
 ...
 async function openFile(id,path){var m=Q('modal');Q('mtitle').textContent=path;Q('mmeta').textContent='';Q('mbody').textContent='loading…';m.style.display='flex';try{var f=await (await fetch('/api/file?id='+encodeURIComponent(id))).json();if(f.error){Q('mbody').textContent='error: '+f.error;return;}Q('mmeta').textContent=[f.lang,f.tier,f.origin,f.license,fmtB(f.bytes),f.provenance].join(' · ');Q('mbody').textContent=f.content;}catch(e){Q('mbody').textContent='failed';}}
