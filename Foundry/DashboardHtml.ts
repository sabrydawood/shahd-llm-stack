   ...
   <section class="view" id="view-train">
    ...
    <div class="grid cards2">
     <div class="card">
      ...
      <div class="row c4" id="t-mix-chat" style="display:none"><div><label class="f"><span class="pill conv">conversation</span> examples</label><input class="i tnum" id="t-conv" type="number" value="4000"></div><div><label class="f"><span class="pill code">code</span> samples</label><input class="i tnum" id="t-code" type="number" value="4000"></div><div><label class="f" title="Stitched 2-3 exchange conversations that teach answering the NEXT question after a finished reply — without them the model collapses on the second message of a conversation. ~15-25% of the mix is a good dose.">Multi-turn</label><input class="i tnum" id="t-multiturn" type="number" value="3000"></div><div><label class="f" title="Warm start: seed the chat model with a pretrained base model's weights instead of random init. The base must have the SAME architecture (embed/layers/heads/context) — its tokenizer is reused verbatim.">From base (warm start)</label><select class="i" id="t-from"><option value="">◇ from scratch</option></select></div></div>
